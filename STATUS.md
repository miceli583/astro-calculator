...
**Last updated by:** Claude PM (two orphaned `mira/*` branches pushed to origin and closed out as already-merged — see Recent changes)
...
| Date       | Author          | Change                                                                                                                                           |
| ---------- | --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| 2026-08-14 | Claude PM       | **Two orphaned `mira/*` branches pushed to origin, then closed out as already-merged (`t_912d6f8b`; no PR, no merge, nothing deleted)** — `retire-claude-seat.sh` had surfaced `mira/env-validation` (a80fdc2) and `mira/claude-md-refresh` (8d9f44f) as local-only commits "one disk failure from gone", and the card sat blocked a day on a keep/drop question. Both were pushed **as-is** first — no squash, rebase or amend — and the remote SHAs verified identical, so durability was settled before any judgement. The judgement then inverted the card's premise: **neither branch was unmerged work**. Both are the head refs of PRs that merged into dev on 2026-07-16 — `mira/claude-md-refresh` → PR #4 → `4cf5dd5`, `mira/env-validation` → PR #5 → `714491d` — and the remote branches were deleted post-merge as usual, which is precisely what left the local ones orphaned with `[origin/…: gone]` tracking. `git cherry dev` marks **both** commits `-` (patch already upstream). Confirmed file-by-file rather than taken on the commit message: `src/env.js` in dev is **byte-identical** to the branch's, as are `layout.tsx`, `robots.ts`, `sitemap.ts` and `instrumentation.ts`; the only three files that differ do so because dev moved *forward* — `package.json` (0.1.0 → 0.3.0), `health/route.ts` (`env.npm_package_version` superseded by `API_VERSION` from `@/lib/version`), `ephemeris/client.ts` (still imports `env`, plus the F1/F2/F5 accuracy work). A PR from that branch would have **regressed** dev on two counts, so opening one to look productive was the wrong move; same for the CLAUDE.md branch, whose content dev strictly supersedes (15 → 16 endpoints, composite, aspect patterns, chart ruler, transit matrix, 22 → 24 test files). Both branches stay on origin as a record. Worth carrying forward: the surfacing script flags "local branch with no remote" as at-risk work without checking whether the commits are already upstream — a `git cherry` test before surfacing would have closed this at source instead of costing a day of blocked-card time. No code touched, no tests run (none needed — zero source change). |
